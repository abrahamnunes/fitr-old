...
%
% INPUTS: (`$` denotes optional fields)
%   results = structure with the results generated by subjects
%
%   model       = structure of the model to be fit, with following fields:
%       .name                = model label (string)
%       .lik                 = likelihood function handle
%       .K                   = integer number of parameters
%       .param               = [1 by K] structure of parameters
%       .param(k).name       = 'Parameter Name' string
%       .param(k).lim        = bounds of parameter [low, high]
%      $.param(k).hyperparam = [1 by M] vector of hyperparameters
%      $.param(k).hyperbound = [1 by M] vector hyperpar bounds [low, high]
%      $.param(k).pdf        = probability density function of
%                              parameter (accepts {x, hyperparam})
%
%   fitoptions = structure containing options for model fitting
%       .iters      = integer number of iterations
%       .nstarts    = integer number of random starting points for the
%                      optimizer
%
%
% OUTPUTS:
